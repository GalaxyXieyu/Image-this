import { Home, Wand2, Layers, Wrench, Sparkles, Droplets, ZoomIn, Expand, Image as ImageIcon, Settings as SettingsIcon } from "lucide-react";
...
export const PRIMARY_ITEMS: PrimaryItem[] = [
  {
    ...
    icon: Wand2,
    match: (p) =>
      p === "/" || p.startsWith("/workspace") || p.startsWith("/combo") || p.startsWith("/tools"),
  },
...
export const WORKSPACE_SEGMENTS = [
  { label: "场景生图", href: "/workspace/scene", icon: Wand2, match: (p: string) => p.startsWith("/workspace/scene") || p.startsWith("/workspace/listing-set") },
  { label: "工作流", href: "/combo", icon: Layers, match: (p: string) => p.startsWith("/combo") },
  { label: "工具箱", href: "/tools", icon: Wrench, match: (p: string) => p.startsWith("/tools") },
];
